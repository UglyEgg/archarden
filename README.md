...
- `--ssh-port <port>`: set the SSH daemon port (default 1731).
...
- Hardens SSH via `/etc/ssh/sshd_config.d/10-hardening.conf` and moves the daemon to a configurable port (**default 1731**) with key-only auth. Validation uses `sshd -t` and the port is verified before firewall changes.
...
1. Run `./harden` with your public key. Use `--ssh-port <port>` to pick a custom port (default 1731) and `--keep-ssh-22` for a transition period if desired.
...
```bash
ssh -L 8181:127.0.0.1:8181 user@server -p 1731  # replace port if you used --ssh-port
```
